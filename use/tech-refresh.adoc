...
summary: If you have existing assets in place and want to have an assessment to identify whether a system refresh is recommended, you can use Lifecycle planning system refresh option. The process takes an assessment of your current workloads and assets and provides recommendations to you on how to proceed. Or, you can try out a workload simulation.  
---

= Evaluate a system refresh by using Lifecycle planning
:hardbreaks:
:icons: font
:imagesdir: ../media/use/

[.lead]
If you have existing assets in place and want to determine whether a system refresh is recommended, you can use the Lifecycle planning System refresh options. You can either review a short assessment of your current workloads and get recommendations, or if you have sent AutoSupport logs to NetApp within the past 90 days, the service can provide a workload simulation to see how your workloads perform on new hardware. 

* <<Take a refresh assessment>>
* <<Simulate workloads on new hardware>>

Systems are considered candidates for a system refresh based on the following: 
...
+
image:tech-refresh.png[System refresh page showing the system refresh candidates]
. Look at the *System refresh candidates* tab, which lists all the systems that might benefit from a system refresh. 
. Select the system on which you want to obtain an assessment. 
. From the Actions column, select *Evaluate refresh*. 
+
image:tech-refresh-systems.png[Systems to be included in the system refresh]
...
+
image:tech-refresh-requirements-environments4.png[System refresh Requirements page showing environment choices]
...
You can review the system refresh assessments that you already took and review the recommendations again.

.Steps

. From the NetApp Console left navigation, select *Storage* > *Lifecycle planning* > *System refresh*. 

. Select the *System refresh candidates* tab, which lists all the systems on which you took a system refresh assessment. Then, from the Actions menu on the row for that system, select to view the assessments and recommendations for that system. 
...
. From the NetApp Console left navigation, select *Governance* > *Lifecycle planning* > *System refresh*. 
+
image:tech-refresh-list2.png[System refresh page showing the System refresh candidates tab]
. Look at the *System refresh candidates* tab, which lists all the systems that might benefit from a system refresh. 
. Select the system on which you want to obtain an assessment. 
